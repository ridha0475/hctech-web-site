...
- [ ] **Refonte du calculateur — BLOQUÉE, décision client attendue.** Le taux de 5 ‰ affiché est contredit par la fiche constructeur GECO (0,90–1,15 ‰) et dépasse le plafond physique d'une cuve. La refonte est spécifiée (fourchette au lieu d'un taux unique, question « reprise des vapeurs à la livraison » qui change le résultat, sort du curseur « volume de cuve » qui n'entre aujourd'hui dans aucun calcul) mais **rien n'est codé** : il manque la fourchette que le client veut publier. Brief et justification dans les notes internes, hors de ce dépôt. Ne pas publier de chiffre d'ici là.
...
- [x] **Performance** — les 7 images de contenu sont converties en WebP et redimensionnées d'après leur taille d'affichage réelle mesurée à 1440 px (×2 rétina) : 12,9 Mo de PNG → 696 Ko, soit −95 % sur le poids servi. Le logo reste en PNG (92 Ko, 29 références, sert aussi de favicon). Les PNG d'origine sont encore versionnés mais plus référencés nulle part : aucun visiteur ne les télécharge, on peut les retirer du dépôt pour alléger le déploiement. Reste : `srcset` pour les petits écrans et une mesure réelle du chargement mobile — reporté avec le reste de la passe mobile.
